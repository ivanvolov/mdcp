...
// Trading API. 0.002 WETH (2000000000000000 raw, 18 dec) -> USDC, 0.5% slippage.
...
// operation (0.002 WETH -> USDC, 0.5% slippage). Where the skill calls for
// AskUserQuestion before broadcasting an approval or the swap itself, that
// confirmation is what's already been given for this run — noted here
...
const AMOUNT_RAW = "2000000000000000"; // 0.002 WETH, 18 decimals
...
async function main() {
  ...
  const usdcBefore: bigint = await publicClient.readContract({
    address: USDC,
    abi: erc20Abi,
  ...
  saveState("weth-usdc-1-check_approval.json", approvalData);
  ...
  if (approvalData.approval) {
    ...
    saveState("weth-usdc-1b-approval-receipt.json", {
      hash: approvalHash,
  ...
  saveState("weth-usdc-2-quote.json", quoteResponse);
  ...
  if (quoteResponse.permitData && typeof quoteResponse.permitData === "object") {
    ...
    saveState("weth-usdc-3-signature.json", { primaryType: derivedPrimaryType, signature });
  } else {
  ...
  saveState("weth-usdc-4-swap.json", swapData);
  ...
  //  one swap of 0.002 WETH -> USDC, 0.5% slippage.)
  ...
  const usdcAfter: bigint = await publicClient.readContract({
    address: USDC,
    abi: erc20Abi,
    functionName: "balanceOf",
    args: [account.address],
  ...
  const summary = {
    ...
    usdcBefore: formatUnits(usdcBefore, 6),
    usdcAfter: formatUnits(usdcAfter, 6),
    usdcReceived_raw: usdcReceived.toString(),
    usdcReceived_human: formatUnits(usdcReceived, 6),
    routing: quoteResponse.routing,
  };
  saveState("weth-usdc-5-summary.json", summary);
