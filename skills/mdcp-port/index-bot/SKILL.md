...
4. Delegates each buy to the mdcp `execute` capability on the target chain.
...
- **mdcp-execute**: the only execution path. The whole basket — every leg's quote, allowance, approval and swap — is expressed as one TypeScript program handed to `execute` and runs inside the sandbox. Do not reimplement any of it.
...
2. Compute the target notional per leg: `legNotional = totalSize * weight`. Price any cross-token sizing with Uniswap quotes (`uniswap.apiQuote` via mdcp), never an external feed or venue-specific API.
...
Express every leg as ONE program passed to `execute`: loop the basket, quote and swap each leg with `uniswap.apiSwap`. The gateway runs the documented Trading API flow (`check_approval` then `quote` then permit then `swap`, then sign and broadcast) inside the sandbox.
...
3. Read current on-chain balances for each leg with `wallet.balances` and value them in the funding token using `uniswap.apiQuote` to get current weights. Do not use venue-specific APIs or external price feeds; see `../../references/execution-model.md` (Data and pricing).
...
6. Delegate each adjusting swap to `execute` exactly as in Step 4.
