...
This is a thin strategy layer. It owns cadence, idempotency, the optional condition check, and guardrails. It does NOT own execution. Every quote, approval, swap, and signature is delegated to the mdcp `execute` capability. The skill never builds quote, approval, swap, or signing logic itself.
...
- **mdcp-execute**: the only execution path. The entire run is expressed as one TypeScript program handed to `execute`; quoting, allowance checks, approval and the swap all happen inside the sandbox. Do not reimplement any of it.
...
Hand execution to mdcp. Express steps 2 through 8 as ONE program passed to `execute`, including the conditionals: the sandbox runs the whole strategy next to the chain and returns only the result. The skill MUST NOT build quote, approval, swap, or signing logic of its own; it only supplies parameters (tokenIn, tokenOut, amount) and reads back the result.
...
| Mode                | Behavior                                                                                                                                                                   |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `confirm` (default) | The gateway suspends the program before broadcasting and returns the full transaction plan. Approve it with one `resume` call. This works with no human present, so it is also the scheduled-run default. |
